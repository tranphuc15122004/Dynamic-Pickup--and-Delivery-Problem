...
def inter_couple_exchange(vehicleid_to_plan: Dict[str , List[Node]], id_to_vehicle: Dict[str , Vehicle] , route_map: Dict[tuple , tuple] , is_limited : bool = False , mode = 'total'):    
    is_improved = False

    dis_order_super_node : Dict[int, Dict[str , Node]] = get_UnongoingSuperNode(vehicleid_to_plan , id_to_vehicle)
    ...
    cost0 = cost_of_a_route(route_node_list, vehicle , id_to_vehicle , route_map , vehicleid_to_plan, mode)
    min_cost = cost0
...
def block_exchange(vehicleid_to_plan: Dict[str , List[Node]], id_to_vehicle: Dict[str , Vehicle] , route_map: Dict[tuple , tuple] , is_limited : bool = False , mode = 'total'):
    is_improved = False
    dis_order_super_node : Dict[int, Dict[str , Node]] = get_UnongoingSuperNode(vehicleid_to_plan , id_to_vehicle)
    ls_node_pair_num = len(dis_order_super_node)
    if ls_node_pair_num == 0:
    ...
    if mode == 'distance':
        origin_cost = factorial_costs_of_an_individual(id_to_vehicle , route_map , vehicleid_to_plan)[0]
    elif mode == 'overtime':
        origin_cost = factorial_costs_of_an_individual(id_to_vehicle , route_map , vehicleid_to_plan)[1]
...
def block_relocate(vehicleid_to_plan: Dict[str , List[Node]], id_to_vehicle: Dict[str , Vehicle] , route_map: Dict[tuple , tuple] , is_limited: bool = False , mode = 'total'):
    is_improved = False
    dis_order_super_node : Dict[int, Dict[str , Node]] = get_UnongoingSuperNode(vehicleid_to_plan , id_to_vehicle)
    ls_node_pair_num = len(dis_order_super_node)
    if ls_node_pair_num == 0:
    ...
    for before_key , before_block in block_map.items():
        ...
        for index, (vehicle_id, vehicle) in enumerate(id_to_vehicle.items(), start=1):
            ...
            for i in range(insert_pos, node_list_size + 1):
                ...
                current_cost = cost_of_a_route(route_node_list, vehicle , id_to_vehicle , route_map , vehicleid_to_plan , mode)
...
def multi_pd_group_relocate(vehicleid_to_plan: Dict[str , List[Node]], id_to_vehicle: Dict[str , Vehicle] , route_map: Dict[tuple , tuple] , is_limited : bool = False, mode = 'total'):
    is_improved = False
    ...
    for idx, pdg in dis_order_super_node.items():
        ...
        cost_after_insertion = single_vehicle_cost(route_node_list , vehicle , route_map , mode)
        ...
        cost_before_insertion = single_vehicle_cost(route_node_list , vehicle , route_map , mode)
        curr_cost_detal = cost_after_insertion - cost_before_insertion
        
        min_cost_delta, best_insert_pos_i, best_insert_pos_j, best_insert_vehicle_id = dispatch_order_to_best(node_list , cp_vehicle_id2_planned_route , id_to_vehicle , route_map, mode)
    ...
    for i in range(ls_node_pair_num):
        if new_cost_delta[i] != math.inf:
            ...
            if not mask[before_vehicle_idx] and not mask[new_vehicle_idx]:
                ...
                cost0 = cost_of_a_route(before_route_node_list, before_vehicle , id_to_vehicle , route_map , vehicleid_to_plan , mode)
                if orgin_cost < 0:
                    orgin_cost = cost0
                ...
                cost1 = cost_of_a_route(new_route_node_list, new_vehicle , id_to_vehicle , route_map , vehicleid_to_plan , mode)
...
def improve_ci_path_by_2_opt(vehicleid_to_plan: Dict[str , List[Node]], id_to_vehicle: Dict[str , Vehicle] , route_map: Dict[tuple , tuple] , begintime , is_limited : bool = False , mode = 'total'):
    is_improved = False
                ...
                if (node_i.pickup_item_list and node_i_plus.delivery_item_list and 
                    ...
                    if pos_k == 0 or pos_k >= j + 1:
                        if (not REV[i + 2][j]):
                            ...
                            cost = cost_of_a_route(temp_route_node_list, vehicle , id_to_vehicle ,route_map , vehicleid_to_plan, mode)
